...
def speak(text):
    ...
    print(f'Bot:  {text}')
def ask():
    user_query = str(input("User(type): "))
    return user_query
...
def OpenApp(app_name):
    speak(f"Opening {app_name.removeprefix('open')}")
    AppOpener.open(app_name,output=False,match_closest=True)
   # speak(f"Opened {app_name.removeprefix('open')}!")
def drink_Water():
...
def wiki_search(user_query):
    ...
    speak("Here\'s your results")
    speak("*results may not be accurate*")
    ctypes.windll.user32.MessageBoxW(0, search_results, f'''Searches {cmd_title}''', 0)
    speak("You may continue:)")

def get_weather(user_query):
    speak("Opening your request, Hang on tight!",)
    user_query = user_query.replace("get", "").capitalize()
    webbrowser.open(f"{user_query}")
    speak("Opened the weather forcast  for your requested city!")
...
def Clear_Cache():
    try:
        ...
        for file in os.listdir(path2):
            os.remove(file)
        speak("Cleared your memory(cache)")
    ...
    while True:
        ...
        if 'open' in user_query:
            ...
            # Opening DIRS
                ...
                speak(f'''Opening {user_query.removeprefix("open").replace("directory",'').strip()} ''')
                os.startfile(f'''C:\\Users\\sahil\\{user_query.removeprefix("open").replace("directory"or"dir",'').strip()}''')
                speak(f'''Opened {user_query.removeprefix("open").replace("directory"or"dir",'').strip()} Dir''')

            else:
                speak("What do you wanna open?")
                for i in list(["directory",'app','website']):
                    if i!=user_query:
                        speak_end(f"{i}?")
                speak("just write `open {dir/app/website} {name}`")
            ...
            if ("whats"and'the') in user_query:
                ...
                strTime = datetime.datetime.now().strftime("%H:%M")
                speak(f"if you asked about `what\'s the time` \n then it\'s {strTime}")
        elif 'search' in user_query:
            if 'module' in user_query:
                user_query = user_query.replace("search module ", "").capitalize()
                speak("Opening `docs.python` , `The Python Package Index`  and `python's official website' for you:)")
            ...
            speak(f"doing your research about `{query}`")
            speak("Opening browser for the results!")
            time.sleep(1)
        ...
        # engaging in casual converstional
            ...
            speak(random.choice(exiting_code))
            exit()
...
#     file.close()
    ...
    #     speak("you can give your feedback now:)")
    #     give_feedback = ask()
